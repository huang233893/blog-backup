...
<head>
	...
	<style type="text/css">
		...
		.copy-btn-container {
		  /* 背景颜色从右向左，从#333向透明转变，从3/4的位置开始渐变 */
	...
	<script type="text/javascript">
        ...
        function decodeSafeUrlParam(paramStr) {
            // 将 URL 安全的字符还原为标准的 Base64 字符
            const base64 = paramStr.replace(/-/g, '+').replace(/_/g, '/');

            // 解码 Base64 后再还原 URL 编码
		...
		if (isWeChat()) {
		    // 微信内置浏览器，调用 WeixinJSBridge 关闭窗口
		    ...
		    // QQ 内置浏览器，调用 mqq.ui.closeWebView 关闭窗口
		    if (typeof mqq !== "undefined" && mqq.ui && mqq.ui.closeWebView) {
		        mqq.ui.closeWebView();
		    } else {
		        window.history.back(); // 退回上一页（如果 WebView 禁用了关闭方法）
		    }
		} else {
		    // 普通浏览器
		    window.opener = null;
		    window.open('', '_self'); // 兼容 Safari
		    window.close();
		}
		}
...
<body>
	<div class="container">
		...
		<div class="loading">
			...
			<div class="countdown-text"> ⚡将在<span id="countdown"></span>秒后跳转，请自行确认链接安全性 </div> {% if countdown > 0 %} <div class="progress-bar">
				<div class="progress" id="progress"></div>
			</div> {% endif %} <div class="button-container">
