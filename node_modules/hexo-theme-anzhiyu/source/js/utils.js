...
const NaoKuo = {
    ...
    try {
      if (!ipLoacation) {
        return new Promise((resolve, reject) => {
          ...
          var url = `https://apis.map.qq.com/ws/location/v1/ip?key=K2ABZ-XLVWW-NZNRD-3S5ST-QDHU5-LHFOI&output=jsonp`;
          script.src = url;
  ...
  showWelcome: (ipStore) => {
    ...
    let dist = NaoKuo.getDistance(113.11320599999999, 23.02971699999999, ipStore.result.location.lng, ipStore.result.location.lat),
      address,
      welcome_info;
    //根据国家、省份、城市信息自定义欢迎语
    //海外地区不支持省份及城市信息
    ...
    //自定义文本需要放的位置
    WelcomeInfo && (WelcomeInfo.innerHTML = `🙋欢迎来自 <strong>${address}</strong> 的小伙伴呢<br>
    😊<strong>${welcome_info}！</strong><br>
    🗺️您距离 <strong>酥米</strong> 约有 <strong>${dist}</strong> 公里哦！<br>
    当前IP地址为：<br>
